...
def evaluate_network(network: RecurrentNetwork) -> float:
    ...
    for episode in range(num_episodes):
        ...
        while not terminated:
            ...
            # 输出log文件
            log_file = [state_list, action, reward, nn_output]
            logger = logger_config(log_file)
            logger.debug(f"Episode: {episode}, Step: {step_count}, Action: {action}, Reward: {reward}\n")
